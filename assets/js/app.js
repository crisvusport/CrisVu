...
function initZoom(){
  ...
  let nw = 1, nh = 1;              // kích thước ảnh gốc
  let baseScale = 1;               // scale để ảnh vừa khung
  let scale = 1, tx = 0, ty = 0;   // scale tuyệt đối + vị trí góc trên-trái (px)
  const pointers = new Map();
  let prevDist = 0, prevMid = null, panLast = null;
  let lastTap = 0, lastPointerType = "mouse";
  ...
  function clampPan(){
    ...
    if(iw <= s.width) tx = (s.width - iw) / 2;
    else tx = Math.min(0, Math.max(s.width - iw, tx));
    if(ih <= s.height) ty = (s.height - ih) / 2;
    else ty = Math.min(0, Math.max(s.height - ih, ty));
  }
  function fitView(){
    const s = stageRect();
    baseScale = Math.min(s.width / nw, s.height / nh) || 1;
    scale = baseScale;
    clampPan(); apply();
  }
  function zoomAt(cx, cy, factor){
    const s = stageRect();
    const px = cx - s.left, py = cy - s.top;
    const MIN = baseScale, MAX = baseScale * 8;
  ...
  window.openZoom = (src) => {
    ...
    img.onload = () => { nw = img.naturalWidth || 1; nh = img.naturalHeight || 1; fitView(); };
    img.src = src;
    if(img.complete && img.naturalWidth){ nw = img.naturalWidth; nh = img.naturalHeight; fitView(); }
  };
  function closeZoom(){ zoom.classList.remove("open"); img.src = ""; }
  ...
  // lăn chuột: phóng to đúng vào vị trí con trỏ
  ...
  stage.addEventListener("pointerdown", e => {
    ...
    if(pointers.size === 1){
      ...
      if(e.pointerType === "touch"){            // điện thoại: chạm 2 lần
        const now = Date.now();
        if(now - lastTap < 300){ lastTap = 0; (scale > baseScale*1.05) ? fitView() : zoomAt(e.clientX, e.clientY, 2.5); }
        else lastTap = now;
      }
    } else if(pointers.size === 2){
      const p = [...pointers.values()];
      prevDist = dist(p[0], p[1]); prevMid = mid(p[0], p[1]);
    }
  });
  // máy tính: nhấp đúp để phóng to đúng điểm / thu về vừa khung
  stage.addEventListener("dblclick", e => {
    if(lastPointerType === "touch") return;
    (scale > baseScale*1.05) ? fitView() : zoomAt(e.clientX, e.clientY, 2.5);
  });
  window.addEventListener("pointermove", e => {
    ...
    if(pointers.size === 2){
      ...
      if(prevDist){
        zoomAt(m.x, m.y, d / prevDist);          // chụm để phóng to quanh điểm giữa
        tx += m.x - prevMid.x; ty += m.y - prevMid.y; clampPan(); apply();  // và di chuyển theo tay
      }
  ...
  function up(e){
    ...
    if(pointers.size === 1){ const q = [...pointers.values()][0]; panLast = { x:q.x, y:q.y }; prevDist = 0; }
    if(pointers.size === 0){ panLast = null; prevDist = 0; }
  }
  window.addEventListener("pointerup", up);
  window.addEventListener("pointercancel", up);
  window.addEventListener("resize", () => { if(zoom.classList.contains("open")) fitView(); });
}
